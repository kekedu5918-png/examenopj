...
export default async function ProgressionPage() {
  ...
  return (
    <AccountDashboardSection>
      <h1 className='text-2xl font-bold text-ds-text-primary dark:text-slate-50'>Statistiques et progression</h1>
      <p className='text-ds-text-muted dark:text-slate-300'>Suivi global de ton avancement, streaks, badges et révisions prioritaires.</p>
      ...
      <Card className='bg-ds-bg-elevated dark:bg-slate-900'>
        <CardHeader>
          <CardTitle className='text-ds-text-primary dark:text-slate-100'>Score moyen (révisions en base)</CardTitle>
        </CardHeader>
        <CardContent>
          <div className='text-4xl font-bold text-blue-600 dark:text-blue-300'>{stats.averageScore.toFixed(2)} / 5</div>
          <p className='mt-2 text-sm text-ds-text-muted dark:text-slate-300'>
            Basé sur les entrées <code className='rounded bg-slate-200 px-1 text-xs dark:bg-slate-800'>user_progress</code>{' '}
            liées aux
      ...
      <Card className='bg-ds-bg-elevated dark:bg-slate-900'>
        <CardHeader>
          <CardTitle className='text-ds-text-primary dark:text-slate-100'>Flashcards synchronisées (compte)</CardTitle>
        </CardHeader>
        <CardContent>
          {flashSummary.totalCards === 0 ? (
            <p className='text-sm text-ds-text-muted dark:text-slate-400'>
              Aucune carte enregistrée en base. Révise sur la page{' '}
              <span className='text-ds-text-primary dark:text-slate-200'>Flashcards</span> en
              étant connecté(e) : chaque réponse est enregistrée (même clé carte + portée qu’en local).
            </p>
          ) : (
            <div className='space-y-3 text-sm text-ds-text-muted dark:text-slate-300'>
              <p>
                <span className='font-semibold text-ds-text-primary dark:text-slate-100'>{flashSummary.totalCards}</span>{' '}
                cartes classées au total
                (toutes portées confondues).
              </p>
              <ul className='grid grid-cols-3 gap-2 text-center'>
                <li className='rounded-lg bg-emerald-500/10 py-2 text-emerald-800 dark:text-emerald-200'>
                  <span className='block text-lg font-bold'>{flashSummary.know}</span>
                  <span className='text-xs text-emerald-700/90 dark:text-emerald-300/80'>Je sais</span>
                </li>
                <li className='rounded-lg bg-amber-500/10 py-2 text-amber-900 dark:text-amber-200'>
                  <span className='block text-lg font-bold'>{flashSummary.review}</span>
                  <span className='text-xs text-amber-800/90 dark:text-amber-300/80'>À revoir</span>
                </li>
                <li className='rounded-lg bg-rose-500/10 py-2 text-rose-900 dark:text-rose-200'>
                  <span className='block text-lg font-bold'>{flashSummary.dontKnow}</span>
                  <span className='text-xs text-rose-800/90 dark:text-rose-300/80'>Je ne sais pas</span>
                </li>
              </ul>
      ...
      <Card className='bg-ds-bg-elevated dark:bg-slate-900'>
        <CardHeader>
          <CardTitle className='text-ds-text-primary dark:text-slate-100'>Derniers quiz (compte connecté)</CardTitle>
        </CardHeader>
        <CardContent>
          {quizAttempts.length === 0 ? (
            <p className='text-sm text-ds-text-muted dark:text-slate-400'>
              Aucune session enregistrée pour l’instant. Les parties terminées sur la page{' '}
              <span className='text-ds-text-primary dark:text-slate-200'>Quiz</span> sont enregistrées ici lorsque tu es connecté(e).
            </p>
          ) : (
            <ul className='space-y-3 text-sm text-ds-text-muted dark:text-slate-300'>
              {quizAttempts.map((row) => (
                <li
                  key={row.id}
                  className='flex flex-wrap items-baseline justify-between gap-2 border-b border-ds-border pb-3 last:border-0 last:pb-0 dark:border-slate-800'
                >
                  <span className='font-medium text-ds-text-primary dark:text-slate-200'>{formatQuizMode(row)}</span>
                  <span className='tabular-nums text-cyan-700 dark:text-cyan-300'>
                    {row.score}/{row.total} ({Number(row.percent).toFixed(0)} %)
                  </span>
                  <span className='w-full text-xs text-ds-text-muted dark:text-slate-500'>
...
function StatCard({ title, value }: { title: string; value: string }) {
  return (
    <Card className='bg-ds-bg-elevated dark:bg-slate-900'>
      <CardHeader>
        <CardTitle className='text-sm font-medium text-ds-text-muted dark:text-slate-300'>{title}</CardTitle>
      </CardHeader>
      <CardContent>
        <div className='text-3xl font-bold text-ds-text-primary dark:text-slate-100'>{value}</div>
      </CardContent>
    </Card>
  );
}
