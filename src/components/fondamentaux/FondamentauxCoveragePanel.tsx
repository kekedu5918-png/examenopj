      ...
      <Collapsible open={open} onOpenChange={setOpen}>
        <GlassCard padding='p-6' className='border-white/10'>
          <CollapsibleTrigger className='flex w-full items-center justify-between gap-3 text-left'>
            ...
            <span
              className={cn(
                'flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-white/10 bg-white/[0.04] transition',
                open && 'bg-white/[0.08]',
              )}
            >
              <ChevronDown className={cn('h-5 w-5 text-slate-300 transition-transform', open && 'rotate-180')} aria-hidden />
            </span>
          </CollapsibleTrigger>
          <CollapsibleContent>
