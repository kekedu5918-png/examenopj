...
const cards = [
  ...
  },
  {
    ...
    num: '03',
    title: 'ORAL — MISE EN SITUATION',
    description:
      '40 min de préparation sur un sujet tiré au sort ; oral devant magistrat et commissaire (déroulé + questions)',
    tags: ['40 min prép.', 'Jury', 'Questions'],
          ...
          {cards.map((card, i) => (
            ...
            >
              <Link
                href={card.href}
                className={cn(
                  'group relative flex h-full flex-col overflow-hidden rounded-2xl border border-white/[0.08] bg-white/[0.02] p-6 transition-all duration-300 motion-safe:hover:-translate-y-1',
                  card.hoverBorder,
                  card.hoverShadow
                )}
              >
                ...
                <h3 className='text-xl font-bold text-examen-ink'>{card.title}</h3>
                <p className='mt-3 flex-1 text-sm leading-relaxed text-examen-inkMuted'>{card.description}</p>
                ...
                <ArrowRight className='absolute bottom-6 right-6 h-5 w-5 text-examen-inkMuted opacity-0 transition-all group-hover:translate-x-0.5 group-hover:opacity-100 group-hover:text-examen-ink' />
              </Link>
            </motion.div>
          ))}
        </div>
